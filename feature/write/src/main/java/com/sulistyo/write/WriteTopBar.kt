...
import com.sulistyo.util.toInstant
...
internal fun WriteTopBar(
    ...
    val formattedDate = remember(key1 = currentDate) {
        DateTimeFormatter.ofPattern("dd MMM yyy", Locale.getDefault())
    ...
    val selectedDiaryDateTime = remember(selectedDiary) {
        if (selectedDiary != null) {
            DateTimeFormatter.ofPattern(
                "dd MMM yyyy, hh:mm a",
                Locale.getDefault()
            ).withZone(ZoneId.systemDefault()).format(selectedDiary.date.toInstant())
        } else {
            "Unknown"
        }
    }
